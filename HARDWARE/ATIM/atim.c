...
void ATIM_Configuration(void)
{
	...
	ATIM_InitStruct.BufferState = DISABLE;								//ARR缓存不使能
	...
	ATIM_InitStruct.OverFlowMask = DISABLE;								//上溢出屏蔽使能
	...
	ATIM_OCInitStruct.OCInterruptSelect	= ATIM_OC_IT_UP_COUNTER;	//比较中断模式：向上计数
	ATIM_OCInitStruct.OCInterruptState = ENABLE;					//比较匹配触发中断使能
